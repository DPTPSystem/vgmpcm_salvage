            ...
            if (args.Length != 0)
            {
                if (args.Length > 0)
                {
                    ...
                    if (File.Exists(args[0]))
                    {
                        ...
                        //FileStream pcmf = new FileStream(args[0].Substring(0, 3) + "_out.bin", FileMode.Create);
                        //StreamWriter wpcmf = new StreamWriter(pcmf);
                        int hangszer = 1;
                        // FileStream pcmf = File.Create(args[0].Substring(0, 3) + "_" + hangszer + ".raw");
                        ...
                        for (int i = Header.vgmDataOffset; i < rows.Length; ++i)
                        {
                            switch(rows[i])
                            {
                                ...
                                case 0xE0:  // PCM Start Address
                                    ...
                                    // ellenőrizük, hogy van e már ilyen hangszer elmentve
                                    for (int csx = 0; csx < ChenckSum.Length - 1 && ChackSumFlag == false; csx++)
                                    {
                                        if ((ChenckSum[csx] == CheckSumNumber && ChenckSum[csx] != 0) || ChenckSum[csx] != 0 && CSi == 0) ChackSumFlag = true;
                                    }
                                    ChenckSum[CSi] = CheckSumNumber;
                                    if ((ChackSumFlag == false && CheckSumNumber != 0) || CheckSumNumber != 0 && CSi == 0)   // Ha van egyezés, akkor nem mentjük el az adatokat
                                    {
                                        CSi++;  // ChenckSum buffer indexét inkrementáljuk
                                        FileStream pcmf = File.Create(path + "/" + args[0].Substring(0, 3) + "_" + hangszer + ".raw");
                                        for(Int32 y = 0; y < SDBi-1; y++)
                                        {
                                    ...
                                    {
                                        // létezik már ez a hangszer töröljük buffer tartalmát, nem írjuk ki
                                        for (Int32 z = 0; z < SampleDataBuff.Length - 1; z++) SampleDataBuff[z] = 0;
                                    }
                                    CheckSumNumber = 0;
                                    ChackSumFlag = false;
                        ...
                        Console.WriteLine("Hangszerek száma: {0}", hangszer);
                    ...
                    else
                    {
                        ...
                        Console.WriteLine(DateTime.Now.ToString() + " -> Nem létezik a megadott fájl..(" + args[1] + ")");
                        Console.ResetColor();
                    }
                }
            }
